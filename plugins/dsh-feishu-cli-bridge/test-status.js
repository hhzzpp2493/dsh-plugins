...
try {
  const result = await harness.run('调用 feishu_create_doc 创建文档 title="Agent测试7" content="测试"。报告结果。');
  for (const n of result.notifications) {
    if (n.method === 'session.status') console.log('STATUS:', JSON.stringify(n.params));
  }
