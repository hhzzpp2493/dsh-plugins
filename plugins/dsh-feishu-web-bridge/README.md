...
- 普通文本 / 图片 / 视频 / 音频 / 文件 → 投进该聊天的 web 会话；附件会先下载到 `mediaDir` 并把本地路径交给 agent。图片会要求 agent 调用 `view_image`，由商汤视觉模型独立分析，不依赖当前主模型是否支持图片输入；agent 完成后**以交互卡片回复**（蓝色 `🐋 DeepSeek Harness` 卡片，移动端自适应）。
