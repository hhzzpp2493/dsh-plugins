...
export class EventConsumer extends EventEmitter {
  constructor({ eventKey = 'im.message.receive_v1', identity = 'bot', logger = console, cliBin, cliHome } = {}) {
    super();
  ...
  start() {
    ...
    const child = spawnLarkCli(
      ['event', 'consume', this.eventKey, '--as', this.identity],
      { cliBin: this.cliBin, cliHome: this.cliHome },
    );
    this.child = child;
    let buf = '';
